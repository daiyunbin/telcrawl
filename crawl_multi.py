# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 17:37:16 2020
尝试多线程手机号查询
...
class QiubaiSpider:
    def __init__(self):
        ...
        self.filepath=r'./telsource/telpick.xlsx'
        self.savepath=''
        self.headers = {
    ...
    def get_url_list(self):
        ...
        for i in range(len(data)):
            self.url_queue.put(self.url_temp.format(data[i][3]))
...
#            ret=[info['telno'] for info in content_list]
...
def save(num):
    ...
    savepath_index=1
    start=time.time()
    main_path=r'./output/savefile{}'.format(num)
    #创建文件夹
    if not os.path.exists(main_path):
        os.makedirs(main_path)    
    for index in range(num+2000*0,num+10000,100):      ###改  
        QS.start=index 
        QS.step=100
        if (index-num)%2000==0:
            QS.savepath=r'./output/savefile{}/request{}.json'.format(num,savepath_index+0)  ###改 
            print(QS.savepath)
            savepath_index+=1
        if (index-num)%1000==0:
...
#测试,总共测100个，每次并行搜索10个。每20个生成一个新文件，每50个生成一次新header
#def save_test(num):
#    QS=QiubaiSpider()
#    savepath_index=1
#    for index in range(num,num+100,10):
#        QS.step=10
#        print('index:',index)
#        QS.start=index
#        if (index-num)%20==0:
#            QS.savepath=r'./output/test{}/request{}.json'.format(num+1,savepath_index)
#            print(QS.savepath)
#            savepath_index+=1
##        if (index-num)%50==0:
#            header=get_baidu_headers()
#        QS.headers=header
#        time.sleep(random.uniform(2,4))
#        QS.run()
...
#    save(0)
#######ip池
#ip_url='http://http.tiqu.alicdns.com/getip3?num=20&type=2&pro=&city=0&yys=0&port=1&pack=90111&ts=0&ys=0&cs=0&lb=1&sb=0&pb=4&mr=1&regions=&gm=4'
#ip_data=json.loads(requests.get(ip_url,headers=headers).text)
...
def json2excel(path,df):
    ...
    for example in list_dic:
        if example['telno'] not in ret:
            ...
            l1.append(df[df['tel_num']==int(example['telno'])].iloc[0,0])
